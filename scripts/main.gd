...
var _user_group_visibility : Dictionary = {}   # ← visibilidad elegida por el usuario
var _cull_distance         : float      = 100.0
...
func _on_remove_cam() -> void:
	if _sel_entry == null:
		_status("⚠ Select a camera from the list first.")
		return
...
func _on_img_selected(path: String) -> void:
	if _sel_entry == null:
		_status("⚠ Select a ref camera before loading an image.")
		return
...
func _on_opacity_changed(v: float) -> void:
	...
	if _sel_entry:
		_ref_mgr.set_opacity(_sel_entry, v)
...
func _ensure_selection_controls() -> void:
	var left_vbox := get_node_or_null(
		"RootVBox/WorkHBox/LeftPanel/LeftScroll/LM/LeftVBox"
	) as VBoxContainer
	if left_vbox == null:
		return
	...
	# ── Sync initial values ───────────────────────────────────────────────────
	if _sel_shape_pick:  _sel_shape_pick.select(int(_editor.sel_shape))
	if _sel_mode_pick:   _sel_mode_pick.select(int(_editor.sel_mode))
	if _sel_action_pick: _sel_action_pick.select(int(_editor.sel_action))
	if _sel_border_spin: _sel_border_spin.value = _editor.sel_border_thickness
...
func _set_selection_action(action: int) -> void:
	...
	_sel_apply_btn.text = (
		"Apply Selection (Erase)" if action == BlockEditor.SelAction.ERASE
		else "Apply Selection (Place)"
	)
...
func _refresh_selection_ui() -> void:
	...
	_set_selection_action(
		_sel_action_pick.selected if _sel_action_pick else int(_editor.sel_action)
	)
...
func _connect_signals() -> void:
	...
	_sel_fill_chk.toggled.connect(func(v):
		if _sel_mode_pick == null:
			...
			_set_selection_mode(
				BlockEditor.SelMode.SOLID if v else BlockEditor.SelMode.HOLLOW
			)
	)
	if _sel_shape_pick:
		_sel_shape_pick.item_selected.connect(func(index): _select_selection_shape(index))
	if _sel_mode_pick:
		_sel_mode_pick.item_selected.connect(func(index): _set_selection_mode(index))
	if _sel_action_pick:
		_sel_action_pick.item_selected.connect(func(index): _set_selection_action(index))
	if _sel_border_spin:
		_sel_border_spin.value_changed.connect(func(v):
			_editor.set_sel_border_thickness(int(v))
		)

	_sel_apply_btn.pressed.connect(func():
		_editor.apply_selection()
		_status("Selection applied.")
	)
	_sel_clear_btn.pressed.connect(func():
...
func _get_undo_action_name() -> String:
	match _active_tool:
		BlockEditor.Tool.PAINT:        return "Paint"
		BlockEditor.Tool.ERASE:        return "Erase"
		BlockEditor.Tool.SHAPE_SELECT: return "Selection"
		_:                             return "Edit"
...
func _on_vp_input(event: InputEvent) -> void:
	...
	match _active_tool:
		...
		BlockEditor.Tool.PAINT, BlockEditor.Tool.ERASE, BlockEditor.Tool.SHAPE_SELECT:
			...
			# Forward scroll / orbit to camera even in edit modes
...
func _refresh_view_menu_checks(view_pm: PopupMenu) -> void:
	if not view_pm:
		return
	view_pm.set_item_checked(0, _grid_mesh.visible if _grid_mesh else false)

	var lp = get_node_or_null("RootVBox/WorkHBox/LeftPanel") as Control
	view_pm.set_item_checked(2, lp.visible if lp else true)

	var rp = get_node_or_null("RootVBox/WorkHBox/RightPanel") as Control
	view_pm.set_item_checked(3, rp.visible if rp else true)

	view_pm.set_item_checked(4, _bottom_open)
	view_pm.set_item_checked(6, _occlusion_enabled)

	if _axis_gizmo:
		view_pm.set_item_checked(7, _axis_gizmo.visible)
	else:
...
• [b]Select[/b] → Drag to define area → Apply

[color=#ffcc77]─── Keyboard Shortcuts ───[/color]
• [b]Ctrl + Z[/b] → Undo
• [b]Ctrl + Y[/b] / [b]Ctrl + Shift + Z[/b] → Redo
...
• Mode: Filled, Hollow, Border
• Action: Place or Erase
...
• Right panel → Cameras: Save viewpoints + reference images
• [b]Use[/b] to switch to a camera · [b]Repos.[/b] to copy current view

[color=#88ffaa]Tip:[/color] Scroll, Middle Mouse and Right Mouse always control the camera, even in Paint/Erase mode.
"""
...
func _on_save_selected(path: String) -> void:
	...
	state_data["active_camera"] = (
		_cam_ctrl.get_active_camera().name if _cam_ctrl.get_active_camera() else "aerial"
	)
	...
	if err == OK:
		_status("✅ Saved: %s (%d blocks, %d cameras)" % [
			path.get_file(), _renderer.get_block_count(), cams_data.size()
		])
	else:
...
func _apply_project(data: Dictionary) -> void:
	_renderer.clear_all()
...
func _refresh_groups() -> void:
	...
	for gn in names:
		...
		# ← Leer visibilidad guardada; si no existe aún, usar el estado real del nodo
		eye.button_pressed     = _user_group_visibility.get(gn, gd.node.visible)
		eye.custom_minimum_size = Vector2(20, 0)
		var cap = gn
		eye.toggled.connect(func(v):
			_user_group_visibility[cap] = v   # ← guardar intención del usuario
...
func _on_add_group() -> void:
	...
	d.confirmed.connect(func():
		...
		if n == "":
			n = "Group_%d" % (_renderer.get_group_names().size() + 1)
		_renderer.create_group(n)
		_user_group_visibility[n] = true   # ← nuevo grupo visible por defecto
...
func _on_remove_group() -> void:
	...
	if sel_idx < 0 or sel_idx >= names.size():
		_status("⚠ Select a group first.")
		return
	var gn = names[sel_idx]
	if gn == "Default":
		_status("⚠ Cannot remove the Default group.")
		return
	_renderer.remove_group(gn)
	_user_group_visibility.erase(gn)   # ← limpiar entrada al eliminar el grupo
...
func _set_tool(t: BlockEditor.Tool) -> void:
	...
	match t:
		...
		BlockEditor.Tool.SHAPE_SELECT: _status("Select — drag a shape, adjust mode/height, then Apply")
...
func _set_brush_plane(p: BlockEditor.BrushPlane) -> void:
	...
	match p:
		...
		BlockEditor.BrushPlane.VERTICAL_X: _status("Brush: Vertical facing X")
		BlockEditor.BrushPlane.VERTICAL_Z: _status("Brush: Vertical facing Z")
...
func _update_occlusion_culling() -> void:
	if not _occlusion_enabled:
		# Restore each group to its user-intended visibility
		for gn in _renderer.get_group_names():
	...
	if dist > _cull_distance:
		# Per-group culling: hide only groups with no blocks in range
		for gn in _renderer.get_group_names():
			var gd := _renderer.get_group_data(gn)
			if gd == null:
				continue
			var user_wants : bool = _user_group_visibility.get(gn, true)
			if not user_wants:
				# User explicitly hid this group — keep it hidden regardless
				gd.node.visible = false
				continue
